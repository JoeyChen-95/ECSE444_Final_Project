...
WIFI_Status_t WIFI_OpenServerConnection(int socket,uint32_t Timeout,uint8_t *RemoteIp,uint16_t *RemotePort){
   ES_WIFI_Conn_t conn;
   ES_WIFI_Status_t ret;
   conn.Number = socket;
   ret = ES_WIFI_OpenServerConnection(&EsWifiObj,Timeout,&conn);
   if (RemotePort) *RemotePort=conn.RemotePort;
   if (RemoteIp) memcpy(RemoteIp,conn.RemoteIP,sizeof(conn.RemoteIP));
   return  WIFI_STATUS_OK;
}
